...
import { QuickLinksButton } from 'src/components/common/buttons/QuickLinksButton'
import { QuickLinksButtonIcon } from 'src/components/common/buttons/QuickLinksButtonIcon'
...
const Page: NextPage<Props> = ({ lang, singleton }) => {
  const imageUrl = singleton?.imageUrl ?? ''
  const title = singleton?.title ?? ''
  const facebook = singleton?.iconLinks?.facebook ?? ''
  const instagram = singleton?.iconLinks?.instagram ?? ''
  const youtube = singleton?.iconLinks?.youtube ?? ''
  const email = singleton?.iconLinks?.email ?? ''
  const phone = singleton?.iconLinks?.phone ?? ''
  ...
  return (
    <>
      ...
      <div className='quick-links'>
        ...
        {singleton?.links?.map((link, key) => {
          let url = lang === 'ru' ? link.urlRu : link.urlEn
          url = url.includes('http') ? url : `http://${url}`
          const title = lang === 'ru' ? link.titleRu : link.titleEn
          return (
            <QuickLinksButton href={url} key={key}>
              {title}
            </QuickLinksButton>
          )
        })}
        <div className='row'>
          {facebook ? <QuickLinksButtonIcon href={facebook} faIcon={faFacebookF} /> : null}
          {instagram ? <QuickLinksButtonIcon href={instagram} faIcon={faInstagram} /> : null}
          {youtube ? <QuickLinksButtonIcon href={youtube} faIcon={faYoutube} /> : null}
          {email ? <QuickLinksButtonIcon href={`mailto:${email}`} faIcon={faEnvelope} /> : null}
          {phone ? <QuickLinksButtonIcon href={`tel:${phone}`} faIcon={faPhoneAlt} /> : null}
        </div>
      </div>
